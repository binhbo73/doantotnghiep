    ...
    def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        try:
            ...
            chunks = chunker.chunk_and_embed_enhanced(
                ...
                generate_summaries=should_apply_raptor,
                summary_mode='async'
            )
            ...
            with transaction.atomic():
                ...
                if raptor_applied:
                    try:
                        ...
                        self.logger.info(f"🏗️  [PERSISTENCE] Starting RAPTOR tree construction for {doc.id}")
                        
                        # Initialize clients for tree builder
                        embedding_client = EmbeddingClient()
                        qdrant_client = QdrantClient()
                        ...
                        # This builds level 1-3 summaries and links them
                        raptor_nodes = builder.build_tree(str(doc.id))
                        
                        if raptor_nodes:
                            self.logger.info(f"✅ [PERSISTENCE] Built RAPTOR tree with {len(raptor_nodes)} summary nodes")
                        else:
                            self.logger.warning(f"⚠️ [PERSISTENCE] RAPTOR tree builder returned no nodes for {doc.id}")
                            
                    except Exception as raptor_err:
                        self.logger.error(f"❌ [PERSISTENCE] Failed to build RAPTOR tree: {raptor_err}", exc_info=True)
                        # Don't fail the whole stage if RAPTOR tree building fails, 
                        # as base chunks are already persisted.
