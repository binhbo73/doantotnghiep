# Design System for RAG Enterprise System (Inspired by Linear)
**Project**: Multi-tenant RAG with RBAC, Document Management & AI Chat  
...
## 7. RAG-Specific Components & Interfaces

### Document Card
- Background: `rgba(255,255,255,0.02)` with hover to `rgba(255,255,255,0.04)`
- Border: `1px solid rgba(255,255,255,0.08)`
- Padding: 12px 16px
- Radius: 8px
- Include: Document icon, title (14px, weight 510), size, upload date (small, muted), permission indicator
- Permission badge: Small green pill (`#10b981`) for "Accessible" or red (`#EF4444`) for "Restricted"
- Hover: Show quick actions (preview, share, delete) as ghost buttons
...
- Text: `#ffffff`
- Padding: 12px 16px
- Radius: 12px (rounded, right-aligned)
...
- Text: `#d0d6e0`
- Padding: 12px 16px
- Radius: 12px (rounded, left-aligned)
- Font: 14px weight 400
- Include: Citation badges linking to source documents

**Citation Indicator**
- Inline reference: `[Doc Name] 📄` with link hover
- Pill style: `#7170ff` accent background for quoted sections
- Tooltip on hover: Full document path and chapter

### Permission Indicator Badge
**Access Level Badges**
- **Public**: Green background `#10b981`, "Public" label, 12px weight 510
- **Internal**: Blue background `#5e6ad2`, "Internal", 12px weight 510
- **Restricted**: Red background `#EF4444`, "Restricted", 12px weight 510
- **Personal**: Gray background `#62666d`, "Personal", 12px weight 510

### Search Result Card
- Background: `rgba(255,255,255,0.02)`
- Border: `1px solid rgba(255,255,255,0.05)`
- Padding: 12px 16px
- Include: Document title (16px, weight 510), snippet (14px, muted), relevance score (small label, right)
- Highlight matching query terms in snippet: bright indigo `#7170ff`
- Rank number: Large, muted left side

### User Avatar & Role Badge
- Avatar: 32px circle, `rgba(255,255,255,0.08)` border, center-aligned initial/icon
- Role badge: Small pill attached to avatar, 10px, weight 510
  - "Admin": Red `#EF4444`
  - "Editor": Blue `#5e6ad2`
  - "Viewer": Gray `#62666d`
...
- Use green `#10b981` for "Accessible" states, red `#EF4444` for "Restricted" security states
- Display permission badges on every document and in every chat citation
- Show document source citations in chat responses — never hide AI knowledge sources
...
- **DON'T hide permission information** — always show if user has access to a document
- **DON'T mix up chat citations** — each AI response must clearly link source documents
- **DON'T forget security context** — every document, folder, and user role must be visually distinct
...
**For GitHub Copilot, Cursor, or any LLM:**
1. This `DESIGN.md` file is in the project root
2. **Tell your AI agent:**
   ```
   Use DESIGN.md as the UI reference. Build all components according to this system.
   Every UI element should:
   - Use Linear-inspired dark theme (#0f1011 backgrounds, #f7f8f8 text)
   - Use purple accent (#5e6ad2) for primary actions
   - Include permission badges for security features
   - Display document citations in chat responses
   - Apply responsive breakpoints for mobile/desktop
   ```

### Color Quick Reference

**Backgrounds**: `#0f1011` (panels), `#191a1b` (cards), `rgba(255,255,255,0.02-0.05)` (transparency)  
**Text**: `#f7f8f8` (primary), `#d0d6e0` (secondary), `#62666d` (muted)  
**Accents**: `#5e6ad2` (brand), `#7170ff` (interactive), `#10b981` (success), `#EF4444` (danger)  
**Borders**: `rgba(255,255,255,0.05)` to `rgba(255,255,255,0.08)` (semi-transparent)

### Template Prompts for Common Screens

**Document Management Page:**
```
Build a document list using DESIGN.md. Include:
- Document cards with title, size, date
- Permission badge (green "Accessible" or red "Restricted")
- Hover actions: Preview, Share, Delete
- Search and filter UI in header
- Responsive grid: 3 columns desktop, 1 column mobile
```

**Chat Interface:**
```
Build a chat page using DESIGN.md. Include:
- User messages in purple bubbles (right-aligned)
- AI responses in subtle cards (left-aligned)
- Citation pills linking to source documents
...
```
Build a permission UI using DESIGN.md. Include:
- User list with role badges (Admin/Editor/Viewer)
- Document access level indicator
- Toggle between Public/Internal/Restricted states
- Use brand purple for primary actions
- Show confirmation modals for security changes
```

### Customization Notes
- Typography: Always enable Inter Variable OpenType features `"cv01", "ss03""`
- Icons: Use minimal, geometric icon set (minimal stroke weight)
- Animations: Smooth fade/scale at 200-300ms, easing: cubic-bezier(0.4, 0, 0.2, 1)
- Spacing: Stick to 8px grid (8px, 16px, 24px, 32px)
- Never deviate from the color palette — consistency is critical for multi-tenant system

### For DevOps/Designers
- This document should be referenced in all PRs with visual changes
- Monitor that permission badges are always visible
- Ensure citations are clickable and lead to source documents
- Test responsive behavior at 640px, 768px, and 1024px breakpoints
